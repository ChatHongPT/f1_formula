...
check_api_response "${OUT_RAW}/driverStandings.json" "드라이버 스탠딩"

run_with_spinner "컨스트럭터 스탠딩 다운로드" \
  curl -s "${BASE}/${YEAR}/constructorStandings.json" -o "${OUT_RAW}/constructorStandings.json"
check_api_response "${OUT_RAW}/constructorStandings.json" "컨스트럭터 스탠딩"

run_with_spinner "드라이버 스탠딩 CSV 생성" \
  bash -c "
    echo 'position,driver,constructor,points,wins' > '${OUT_CSV}/driver_standings.csv'
...
run_with_spinner "컨스트럭터 스탠딩 CSV 생성" \
  bash -c "
    echo 'position,constructor,points,wins' > '${OUT_CSV}/constructor_standings.csv'
